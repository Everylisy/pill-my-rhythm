...
function Calendar() {
  ...
  const [appointments, setAppointments] =
    useRecoilState<Array<Appointments>>(appointmentsAtom);
  const setSupplements = useSetRecoilState<Array<Supplements>>(supplementAtom);
  ...
  useEffect(() => {
    if (user.length === 0) {
      ...
      get(`schedule/?start=${new Date(start)}&finish=${new Date(end)}`).then(
        (res) => {
          setLevel(res.data.checklist);
  ...
  const onAppointmentDeleting = useCallback(
    async (e: any) => {
      ...
      const index = appointments.findIndex(
        (appointments) => appointments.endDate === e.appointmentData.endDate,
      );
          ...
          <nav className="lg:leading-6 mt-7">
            <ScrollView id="scroll">
              ...
              >
                <ul>
                  <div
                    id="supplementsDnDService"
                    className="py-5 px-5 max-w-sm mx-auto bg-white rounded-xl"
                  >
                    {dayHour.map((task) => (
                  ...
                  <div
                    id="biorhythmsDnDService"
                    className="mt-7 bg-white rounded-xl py-5 px-5"
                  >
                    {tasks.map((task) => (
